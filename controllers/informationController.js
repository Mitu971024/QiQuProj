...
module.exports = {
    ...
    getgameInfo:async (ctx,next) =>{
        try{
            let jsondata = await informationDAO.getgameInfo();
            ctx.set('content-type','application/json')
            ctx.body={"code": 200, "message": "ok", data:jsondata};
        }catch(err){
            ctx.body ={"code": 500, "message": err.toString(), data:[]};
    ...
    getrecommendInfo:async (ctx,next) =>{
        try{
            let jsondata = await informationDAO.getrecommendInfo();
            ctx.set('content-type','application/json')
            ctx.body={"code": 200, "message": "ok", data:jsondata};
        }catch(err){
            ctx.body ={"code": 500, "message": err.toString(), data:[]};
